...
// Capture native sync C functions before overwriting with JS Promise wrappers
...
const reportError = (err: unknown): void => {
  try {
    ...
    try { bridge.reportError?.("Backend error"); } catch {}
  }
};

// --- Context marker ---
try {
  Object.defineProperty(g, Symbol.for("zapp.context"), {
    value: "backend",
    enumerable: false,
    configurable: false,
    writable: false,
  ...
  (g as Record<symbol, unknown>)[Symbol.for("zapp.context")] = "backend";
}

// --- App config cache ---
type CachedConfig = { name: string; applicationShouldTerminateAfterLastWindowClosed: boolean; webContentInspectable: boolean; maxWorkers?: number };
const initialConfig = (g as Record<string, unknown>).__zappConfig as CachedConfig | undefined;
const appConfig: CachedConfig = initialConfig
  ? { ...initialConfig }
  : { name: "Zapp App", applicationShouldTerminateAfterLastWindowClosed: false, webContentInspectable: true };
try { delete (g as Record<string, unknown>).__zappConfig; } catch {}

(bridge as Record<string, unknown>).getConfig = (): CachedConfig => ({ ...appConfig });
(bridge as Record<string, unknown>).mergeConfig = (partial: Partial<CachedConfig>): void => {
  if (partial && typeof partial === "object") {
    if (typeof partial.name === "string") appConfig.name = partial.name;
    if (typeof partial.applicationShouldTerminateAfterLastWindowClosed === "boolean") appConfig.applicationShouldTerminateAfterLastWindowClosed = partial.applicationShouldTerminateAfterLastWindowClosed;
    if (typeof partial.webContentInspectable === "boolean") appConfig.webContentInspectable = partial.webContentInspectable;
    if (typeof partial.maxWorkers === "number") appConfig.maxWorkers = partial.maxWorkers;
  }
};
...
{
  const makeLogger = (level: string) => (...args: unknown[]) => {
    const msg = args.map(a => {
      if (typeof a === "string") return a;
      try { return JSON.stringify(a); } catch { return String(a); }
    }).join(" ");
    (bridge as Record<string, unknown>).consoleLog?.(level, msg);
  };
  g.console = {
    log: makeLogger("log"),
...
let zappNextId = 0;
const ctxId = `backend-${Date.now()}-${Math.random().toString(36).slice(2)}`;
const getBackendId = (): string => "__zapp_backend__";
...
(bridge as Record<string, unknown>).syncWait = (request: { key: string; timeoutMs: number | null; signal?: { aborted: boolean; addEventListener?: (t: string, fn: () => void) => void } }): Promise<string> =>
  new Promise((resolve, reject) => {
    ...
    nativeSyncWait?.({ id: requestId, key, timeoutMs: timeout, targetWorkerId: getBackendId() });
    ...
    if (request?.signal?.addEventListener) {
      request.signal.addEventListener("abort", () => {
        ...
        delete syncPending[requestId];
        nativeSyncCancel?.({ id: requestId, targetWorkerId: getBackendId() });
...
(bridge as Record<string, unknown>).syncNotify = (request: { key: string; count: number }): boolean => {
  const key = typeof request?.key === "string" ? request.key.trim() : "";
  if (!key) return false;
  return nativeSyncNotify?.({ key, count: Math.max(1, Math.min(65535, Math.floor(Number(request?.count ?? 1)))), targetWorkerId: getBackendId() }) ?? false;
};

(bridge as Record<string, unknown>).syncCancel = (request: { id: string }): boolean => {
  const id = typeof request?.id === "string" ? request.id.trim() : "";
  if (!id) return false;
  return nativeSyncCancel?.({ id, targetWorkerId: getBackendId() }) ?? false;
};
...
(bridge as Record<string, unknown>).appConfigure = (config: Partial<Record<string, unknown>>): void => {
  if (!config || typeof config !== "object") return;
  bridge.emitToHost?.("__zapp_app_configure", JSON.stringify(config));
};
...
// Register as Symbol.for("zapp.bridge")
try {
...
// --- Bridge callbacks (called from native) ---
