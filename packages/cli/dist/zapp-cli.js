...
async function runGenerate({
  ...
  const targetDir = outDir ? path4.resolve(root, outDir) : path4.join(frontendDir ?? root, "src", "generated");
...
var runInit = async ({
  ...
  await mkdir4(zappDir, { recursive: true });
...
var runPackage = async ({ root, nativeOut }) => {
  ...
  const appName = path8.basename(root) || "ZappApp";
...
var root = path9.resolve(cwd, parseFlag("--root", "."));
var frontendDir = path9.resolve(root, parseFlag("--frontend", "."));
var buildFile = path9.resolve(root, parseFlag("--input", parseFlag("--build-file", "zapp/build.zc")));
var defaultOut = process8.platform === "win32" ? "zapp.exe" : "zapp";
var nativeOut = path9.resolve(root, parseFlag("--out", defaultOut));
var assetDir = path9.resolve(frontendDir, parseFlag("--asset-dir", "dist"));
