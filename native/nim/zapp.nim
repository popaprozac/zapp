...
# The per-TARGET native link surface — the platform .m compile list, the
# framework {.passL.}, and the libzjs link — is GENERATED by the CLI into
# .zapp/zapp_platform.nim (imported below, UnusedImport-suppressed) so the Nim
# build picks darwin-vs-ios sources/frameworks/libzjs by target. It used to be
# hardcoded here (darwin-only): the 18 `{.compile("../platform/darwin/*.m",
# "-fobjc-arc").}` (CALL form — the THIRD arg is per-file clang flags; the TUPLE
# form treats the 2nd elem as the OUTPUT OBJECT NAME, dropping ARC + clobbering
# objects, and window.m's weak WKWebView props REQUIRE ARC), the macOS framework
# passL, and the libzjs.dylib + rpath passL. The zjs.c {.compile.} below stays
# here — it is target-agnostic and gets SDK flags globally.
...
# zapp_platform — the CLI-generated per-target native link surface (.m compile
# list + frameworks + libzjs link, darwin-vs-ios by target). Imported only for
# its compile/passL pragma side-effects (no Nim symbol referenced here), so
# silence UnusedImport like the others. buildNativeNim writes it to .zapp/ and
# passes --path:<.zapp>, so it resolves by name.
import zapp_platform
{.pop.}
...
{.compile("../worker/engines/zjs.c",
          ...
          " -DZAPP_NIM_BUILD").}
# -framework Foundation, -lcompression, the libzjs link (libzjs.dylib + rpath on
# macOS; libzjs_embed.a + -lz on iOS) and the platform frameworks all moved to
