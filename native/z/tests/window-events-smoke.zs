...
function runWindowEventsSmoke(
  ...
  const closeRequested = try events.closeRequested.subscribe(
  ...
  const firstCloseAllowed = events.publishCloseRequested("win-1");
  closeRequested.unsubscribe();
  const secondCloseAllowed = events.publishCloseRequested("win-1");
