...
import { WindowDropPosition } from "../../events.zs";
...
internal class MacOSFileDrops on thread.main {
  ...
  function enter(inout this, in sender: WebKit.NSDraggingInfo): WebKit.NSDragOperation {
    ...
    if (!session.begin(sender.draggingSequenceNumber)) return WebKit.NSDragOperationNone;
    this.session = Option.some(session);
    return this.update(in sender);
  }

  function update(in sender: WebKit.NSDraggingInfo): WebKit.NSDragOperation {
    const session = match (this.active()) { some(value) => value; none => return WebKit.NSDragOperationNone; };
    if (!session.current(sender.draggingSequenceNumber)
      || usize(sender.draggingSourceOperationMask & WebKit.NSDragOperationCopy) == 0) return WebKit.NSDragOperationNone;
    return WebKit.NSDragOperationCopy;
  }

  function exit(inout this): void { this.session = Option.none; }

  function prepare(in sender: WebKit.NSDraggingInfo): boolean {
    if (this.update(in sender) == WebKit.NSDragOperationNone) return false;
    return match (macOSDroppedFiles(sender.draggingPasteboard)) { some(_) => true; none => false; };
  }
  ...
  function perform(inout this, in sender: WebKit.NSDraggingInfo): boolean {
    ...
    const identity = match (this.document.readyIdentity()) { some(value) => value; none => return false; };
    ...
    if (zoom <= 0) { this.exit(); return false; }
    const position = WindowDropPosition({ x: (point.x - bounds.origin.x) / zoom,
    ...
    const script = `(()=>{const b=globalThis[Symbol.for('zapp.bridge')];if(b&&typeof b._onDocumentWindowEvent==='function')b._onDocumentWindowEvent('${identity.token}','files-dropped',${payload})})()`;
