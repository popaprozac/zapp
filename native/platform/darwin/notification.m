...
void darwin_notification_set_bridge_ready(void) {
    ...
    // Flush any buffered notification events
...
@interface ZappNotificationDelegate : NSObject <UNUserNotificationCenterDelegate>
@end

@implementation ZappNotificationDelegate
- (void)userNotificationCenter:(UNUserNotificationCenter*)center
    ...
    // Determine event type and build payload JSON
    NSString* eventName;
    NSString* payload;
    int appEventId;
    if ([actionId isEqualToString:UNNotificationDefaultActionIdentifier]) {
        ...
        // Check for text input (reply field)
        NSString* userText = @"";
    ...
    // Layer 1: Dispatch to native Zen-C callbacks (fires immediately, no WebView needed)
    extern int zapp_app_dispatch(int event_id, const char* data);
    zapp_app_dispatch(appEventId, [payload UTF8String]);

    // Layer 2: Forward to JS bridge (buffered until handler subscribes)
...
static ZappNotificationDelegate* zapp_notif_delegate = nil;
...
const char* darwin_notification_get_permission(void) {
    ...
    // This is fine to block briefly — getNotificationSettings is fast (no UI)
...
// --- Native API (typed, fire-and-forget) ---

// Category accumulator — setNotificationCategories replaces all, so we track the full set
...
void darwin_notification_register_category_typed(
    ...
    // Add text input action if reply field requested
    if (has_reply) {
...
// Register from JSON (bridge path)
void darwin_notification_register_category(const char* category_id, const char* actions_json) {
    ...
    if (actions_json) {
        ...
        // Support both array (actions only) and object (full category)
        NSArray* items = nil;
...
// Show a notification with an optional category (for action buttons).
