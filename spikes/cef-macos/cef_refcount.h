// CEF spike (Task 0) — manual reference-counting macros for CEF C-API structs.
//
// Adapted verbatim-in-spirit from the CEF reference:
//   tests/cefsimple_capi/ref_counted.h
// (https://github.com/chromiumembedded/cef, BSD-licensed). Every cef_*_t the
// spike implements embeds a cef_base_ref_counted_t as its first member and must
// provide atomic add_ref/release/has_one_ref/has_at_least_one_ref. These macros
// generate that boilerplate. Unchanged from upstream except the header guard and
// this comment — the spike's Nim `importc` wrappers must honor this exact ABI.

#ifndef ZAPP_SPIKE_CEF_REFCOUNT_H_
#define ZAPP_SPIKE_CEF_REFCOUNT_H_
...
#endif  // ZAPP_SPIKE_CEF_REFCOUNT_H_
