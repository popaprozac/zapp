...
test("fails explicitly for unsupported image-set and malformed input", () => {
  for (const source of ['a{a:image-set("x.png" 1x)}', 'a{a:url(unclosed}', '/* unterminated', 'a{content:"unterminated']) {
    expect(() => rebaseStyleURLs(source, base)).toThrow("Style experiment:");
  }
});
