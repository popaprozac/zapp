...
type Target = { head?: HTMLHeadElement; end?: Comment; nodes: Map<Source, Source> };
...
export function createStyleMirror(sourceDocument: Document) {
  ...
  return {
    attach(document: Document) {
      ...
      const target: Target = { head: document.head, end: document.createComment("style experiment end"), nodes: new Map() };
