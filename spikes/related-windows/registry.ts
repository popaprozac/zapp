...
const source = join(root, "native/z/tests/related-documents-smoke.zs");
...
for (const frontend of ["native", "stage0"]) {
  ...
  const generated = join(artifacts, `registry-${frontend}.c`);
  await Bun.write(generated, emission.stdout);
  for (const optimization of ["-O0", "-O2"]) {
    const binary = join(artifacts, `registry-${frontend}${optimization}`);
    ...
    const pass = outcome.status === 0 && !outcome.timedOut && outcome.stderr === ""
      && outcome.stdout === "related document registry: identity, readiness, authority, generations, cancellation passed\n";
    results.push({ frontend, optimization, pass, ...outcome });
    console.log(`${pass ? "PASS" : "FAIL"} registry ${frontend} ${optimization}`);
    if (!pass) console.log(outcome);
  }
}
await Bun.write(join(artifacts, "registry-results.json"), JSON.stringify(results, null, 2) + "\n");
