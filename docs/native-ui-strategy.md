...
| SwiftUI accessories **Sub-cycle 2c** — presentation + resize | sidebar/inspector **tile** parity (drop `.ignoresSafeArea`), style-conditional titlebar bleed, create-time pane geometry, and runtime width/resize/collapsible on the SwiftUI path (#660 — **hybrid**: declarative `@Published` resize-lock/collapsible + imperative reach-through width). Collapsible **divider-drag** gating is a known gap (SwiftUI has no API) → follow-up. | ✅ Done (macOS) — see *Pane control* below |
...
**Known gap (follow-up):** SwiftUI's `NavigationSplitView`/`.inspector` have **no clean API to forbid user-collapse**, so `collapsible: false` does not reliably block the **divider-drag collapse** (sidebar drag is glitchy; the inspector divider isn't gated). Tracked for a SwiftUI spike — see the follow-up tasks.
