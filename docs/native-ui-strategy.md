...
| SwiftUI accessories **Sub-cycle 2c** — presentation + resize | sidebar/inspector **tile** parity (drop `.ignoresSafeArea`), style-conditional titlebar bleed, create-time pane geometry, and runtime width/resize/collapsible on the SwiftUI path (#660 — **hybrid**: declarative `@Published` resize-lock/collapsible + imperative reach-through width). Collapsible **divider-drag** gating shipped via an AppKit lock on the backing `NSSplitView` (#665); the toolbar-toggle polish is a follow-up (#668). | ✅ Done (macOS) — see *Pane control* below |
...
**Remaining toolbar-toggle edge (#668, follow-up):** with `canCollapse = false`, macOS surfaces the *native* sidebar toggle inconsistently — it can persist and, when clicked, makes `NavigationSplitView` re-derive the item (resetting `canCollapse`) and momentarily un-lock the drag. The fix is to remove the native toggle (`.toolbar(removing: .sidebarToggle)`) and app-render our own disabled-when-locked toggle — a spike, because `removing:` previously leaked a duplicate across the hosting seam (see #668).
