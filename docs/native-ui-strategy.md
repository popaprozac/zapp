...
**Collapse-gating is durable across relayouts (#673, shipped).** A route change that swaps a pane's DOM without resizing the split's subviews makes `NavigationSplitView` re-derive `canCollapse` back to `true`, which neither `@Published` nor the resize observer catches. Fixed by `PaneItemKVO` — a KVO observer on the split item's `canCollapse` + `minimumThickness` that re-asserts the desired geometry (via the guarded `enforce()`) the instant either is re-derived, regardless of trigger (deferred a runloop to avoid re-entrancy; `enforce()` writes only when different, so it can't loop). Note: the canonical "set `canCollapse=false` once via introspect/`onAppear`" StackOverflow answers structurally can't fix this — they fire on a SwiftUI lifecycle event, which a content-only re-derivation isn't; **observing the property itself** is the lever. `native: { swiftui: false }` (real `NSSplitViewController`) remains available and is inherently free of re-derivation, but the SwiftUI path now holds collapse-gating across route changes too.
