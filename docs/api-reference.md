...
| `icon` | `string` | none | Group icon (`sf:…`/data-URL/path) — renders the collapsed in-toolbar pull-down BUTTON BODY. In icon-only toolbar display mode AppKit shows the collapsed group's *image*, not its `label` — `label` alone still leaves a blank button. Recommend setting both `label` and `icon` on icon-only groups: `icon` for the in-toolbar button, `label` for the overflow entry + customization palette (#744). |
