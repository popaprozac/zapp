...
|---|---|---|---|
| 1 | **Prod macOS build** | Medium | `buildNativeNim` hardcodes `isDev:true` / `embedAssets:false` / `devTools:1` (`native.ts:1133-1135`), ignores `optimize` (`:1257`), emits no brotli asset table (`zapp.nim:176-184` empty) → only a *dev* binary that can't run detached. `createProductionBundle` detects embedded assets via the zc-only `zapp_assets.zc` (`package.ts:50`). Prereq for everything downstream. |
