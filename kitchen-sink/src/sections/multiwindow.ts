...
async function open(
  ...
  try {
    ...
    setResult(
      host,
      `${label} failed — likely needs WindowManager (zc build for now): ${e}`,
    );
  }
}
  ...
  render(host) {
    ...
    host.appendChild(
      card({
        ...
        intro:
          'Open additional windows. Works on both builds now (the WindowManager port landed on Nim). "Sidebar shell" opens a second full native-chrome window — the chrome-on-Nim demo.',
        buttons: [
    ...
    onAct(host, "shell", () =>
      open(host, "sidebar-shell window", () =>
        Window.create({
          title: "Kitchen Sink — Shell 2",
          width: 1000,
          height: 680,
    ...
    onAct(host, "sheet-page", () =>
      open(host, "page sheet", () =>
        Window.create({
          title: "Settings",
          url: "#sheet=settings",
          width: 480,
    ...
    onAct(host, "sheet-form", () =>
      open(host, "form sheet", () =>
        Window.create({
          title: "Quick Add",
          url: "#sheet=quickadd",
          width: 400,
    ...
    onAct(host, "sheet-bottom", () =>
      open(host, "bottom sheet", () =>
        Window.create({
          title: "Drawer",
          url: "#sheet=drawer",
          asSheetOf: win,
    ...
    host.appendChild(
      card({
        ...
        intro:
          "Open demo windows that show backgroundExtension in action. Each window has a full-bleed gradient so the glass effect is clearly visible. macOS 26+ only; earlier versions show a plain sidebar.",
        buttons: [
    ...
    onAct(host, "bg-mirror", () =>
      open(host, "bg-mirror window", () =>
        Window.create({
    ...
    onAct(host, "bg-extend", () =>
      open(host, "bg-extend window", () =>
        Window.create({
    ...
    host.appendChild(
      card({
        ...
        intro:
          "backgroundColor now accepts CSS names, #rgb/#rrggbb/#rrggbbaa, rgb() and rgba(). This window uses a named window color (opaque) and a translucent rgba() sidebar whose alpha lets the window color show through.",
        buttons: [{ act: "color-demo", label: "Open color window" }],
      }),
    );
    onAct(host, "color-demo", () =>
      open(host, "color window", () =>
        Window.create({
    ...
    host.appendChild(
      card({
        ...
        intro:
          "Launch five windows with distinct title-bar / toolbar configurations so they can be compared side by side. Each window names its own config.",
        buttons: [
    ...
    onAct(host, "tb-standard", () =>
      open(host, "standard title bar", () =>
        Window.create({
    ...
    onAct(host, "tb-hidden", () =>
      open(host, "hidden title bar", () =>
        Window.create({
    ...
    onAct(host, "tb-unified", () =>
      open(host, "unified toolbar", () =>
        Window.create({
    ...
    onAct(host, "tb-unified-compact", () =>
      open(host, "unified compact", () =>
        Window.create({
    ...
    onAct(host, "tb-expanded", () =>
      open(host, "expanded toolbar", () =>
        Window.create({
