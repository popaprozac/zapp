...
- **Security** — Declarative permissions, per-window capability profiles, and compiled navigation profiles in `zapp.config.ts`; trusted Z may narrow navigation per request while frontend TypeScript observes the final decision. Also includes FS path allowlists, sandboxed embeds, path traversal prevention, and production DevTools policy. See [`docs/security.md`](docs/security.md).
