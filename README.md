...
- **Security** — Declarative permissions, per-window capability profiles, and compiled navigation profiles in `zapp.config.ts`; trusted Z may narrow navigation per request while frontend TypeScript observes the final decision. Explicit external URL handoff intersects all three policy layers before reaching the OS. Navigation does not imply bridge authority: native routing accepts only the `"self"` main frame and rejects raw subframe messages before decoding. Also includes FS path allowlists, sandboxed embeds, path traversal prevention, and production DevTools policy. See [`docs/security.md`](docs/security.md).
