...
import type { ResolvedConfig, ZappConfigMode } from "./config";
...
export function configuredApplicationQuitOnLastWindowClosed(): boolean {
  return ${config.quitOnLastWindowClosed !== false};
}
...
export function renderZConfiguredWebView(
  ...
  policy: { mode: ZappConfigMode; inspectable?: boolean },
  ...
  const development = policy.mode === "development";
  const inspectable = policy.inspectable ?? development;
  let source = `// AUTO-GENERATED by zapp CLI. Do not edit.\n\n`;
  source += renderZWebViewDeveloperExtras(policy.mode);
  ...
  source += `}\n\n`;
  source += `internal function configuredWebViewInspectable(): boolean {\n`;
  source += `  return ${inspectable};\n`;
  source += `}\n\n`;
...
// This unsupported WebKit preference is a development convenience only. Do
// not emit its selector, KVC key, or exception bridge into production sources.
export function renderZWebViewDeveloperExtras(mode: ZappConfigMode): string {
  const signature = `internal function configureWebViewDeveloperExtras(\n`
    + `  in configuration: WebKit.WKWebViewConfiguration,\n`
    + `  enabled: boolean\n`
    + `): void on thread.main`;
  const imports = `import WebKit from "WebKit/WebKit.h";\n`
    + `import { thread } from "std/thread";\n\n`;
  if (mode !== "development") return `${imports}${signature} {}\n\n`;
  return `${imports}${signature} = raw objc {
  WKPreferences *preferences = configuration.preferences;
  if (![preferences respondsToSelector:NSSelectorFromString(@"_setDeveloperExtrasEnabled:")]) return;
  @try {
    [preferences setValue:@(enabled) forKey:@"developerExtrasEnabled"];
  } @catch (NSException *exception) {
    (void)exception;
    // Inspection through Safari remains available through the public API.
  }
}\n\n`;
}

export function renderZConfiguredFilesystem(
...
export async function buildNativeZ(options: BuildNativeZOptions): Promise<void> {
  ...
  await writeFile(
    path.join(stagedFramework, "configured-filesystem.zs"),
    renderZConfiguredFilesystem(options.config.fs?.allow ?? []),
    "utf8",
  );
  if (desktop) {
  ...
  if (desktop) {
    ...
    await writeFile(
      ...
      renderZConfiguredWebView(
        ...
        injectionEntries,
        { mode: options.mode, inspectable: options.config.webviewInspectable },
        navigationProfiles,
      ),
      "utf8",
    );
