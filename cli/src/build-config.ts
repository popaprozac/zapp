...
/**
 * Render `.zapp/zapp_initial_window.nim` — exposes the app's optional `window`
 * config block as a JSON string the Nim build root parses with
 * windowOptsApplyJson. Empty string when no `window` block (skeleton fallback).
 */
export function renderInitialWindowNim(window: import("./config").InitialWindowConfig | undefined): string {
  let json = "";
  if (window) {
    const obj: Record<string, unknown> = {};
    if (window.title !== undefined) obj.title = window.title;
    if (window.width !== undefined) obj.width = window.width;
    if (window.height !== undefined) obj.height = window.height;
    const pane = (p: import("./config").InitialWindowPaneConfig) => {
      const o: Record<string, unknown> = {};
      if (p.url !== undefined) o.url = p.url;
      if (p.material !== undefined) o.material = p.material;
      if (p.width !== undefined) o.width = p.width;
      if (p.minWidth !== undefined) o.minWidth = p.minWidth;
      if (p.maxWidth !== undefined) o.maxWidth = p.maxWidth;
      if (p.collapsible !== undefined) o.collapsible = p.collapsible;
      if (p.collapsed !== undefined) o.collapsed = p.collapsed;
      return o;
    };
    if (window.sidebar) obj.sidebar = pane(window.sidebar);
    if (window.inspector) obj.inspector = pane(window.inspector);
    if (window.toolbarJson !== undefined) obj.toolbarJson = window.toolbarJson;
    json = JSON.stringify(obj);
  }
  // Embed `json` as a Nim string literal: escape backslash then double-quote.
  // (Match the escaping the sibling render*Nim functions use; the window JSON
  // has no newlines, but follow the established pattern.)
  const nimLit = json.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
  return `## AUTO-GENERATED by zapp build (Nim). Initial-window config for the
## build root; "" when no \`window\` block in zapp.config.ts (skeleton fallback).
proc zapp_window_config_json*(): cstring {.exportc, cdecl.} =
  "${nimLit}".cstring
`;
}
