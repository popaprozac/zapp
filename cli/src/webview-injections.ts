...
export function renderWebviewInjectionsC(
  ...
  const table = entries.length === 0
    ...
    : `static const ZAppWebviewInjection zapp_webview_injections[] = {\n` +
      ...
      `\n};`;
  const existsBody = entries.length === 0
    ? "  (void)profile;\n  return 0;"
    : `  if (profile == NULL) return 0;\n` +
      `  for (size_t index = 0; index < ${entries.length}; index++) {\n` +
      `    if (strcmp(zapp_webview_injections[index].profile, profile) == 0) return 1;\n` +
      `  }\n  return 0;`;
...
int32_t zapp_webview_injection_profile_exists(const char *profile) {
${existsBody}
}
